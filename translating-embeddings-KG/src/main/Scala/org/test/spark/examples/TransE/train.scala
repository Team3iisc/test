package org.test.spark.examples.TransE
...
object train {
  ...
  val LOG = LoggerFactory.getLogger(getClass) 
   ...
   def get_residual( e1:Array[Double],e2:Array[Double],r:Array[Double]) : Array[Double] = {
     val n = e1.length
     var x = new Array[Double](n);
  ...
  def main(args: Array[String]) {
    var strt1 = ((new Date).getTime).toDouble
   ...
   println("time for retrieving entities = %5.8f;".format((end-strt)/1000))
    ...
    var end1 = ((new Date).getTime).toDouble
    println("time = %5.8f;".format((end1-strt1)/1000))
  ...
  val process = (elem: String, weight: Double, sharedVar : SharedVariableSet,  localVar: LocalVariableSet) => {
   ...
   // LOG.info("data:"+e1+" "+rel+" "+e2+" "+e1c+" "+relc+" "+e2c+" ")
    ...
    val X1 = get_residual(w_e1,w_e2,w_r)
    val X2 = get_residual(w_e1c,w_e2c,w_rc)
    ...
    if(sum1+margin  > sum2 ){
      ...
      for(i <- 0 to dim-1){
        delta_1(i) = stepsize*X1(i)
        delta_2(i) = -stepsize*X1(i)
      }
      ...
      for(i <- 0 to dim -1){
        delta_1(i) = stepsize*X2(i)
        delta_2(i) = -stepsize*X2(i)
      }
