package org.test.spark.examples.TransH
...
object train {
   ...
   def get_residual( e1:Array[Double],e2:Array[Double],r:Array[Double],hpr:Array[Double]) : Array[Double] = {
     val n = e1.length
     var x = new Array[Double](n);
     ...
     for (i <- 0 to n-1){
       x(i) = (e2(i)-dot2*hpr(i) - e1(i) + dot1*hpr(i) -r(i))
     }
     return x
   }
   def main(args: Array[String]) {
    ...
    val A = sc.textFile(inputPath + "/A").map(x => {
      val ids = x.split(",")
      (ids(0).toInt, ids(1).split("\\s+").map(_.toDouble))
      }).collectAsMap()
    val A_num = A.size
    ...
    paramRdd.foreachSharedVariable( sharedVar => {
      ...
      for(i <- 0 to relation_num-1){
        sharedVar.declareArray("A" + i, dim) 
        sharedVar.setArray("A" + i, relation.getOrElse(i, null) )
      }
  ...
  val process = (elem: String, weight: Double, sharedVar : SharedVariableSet,  localVar: LocalVariableSet) => {
    ...
    var w_hpr = sharedVar.getArray("A" + rel)
    
    var w_e1c = sharedVar.getArray("e" + e1c)
    var w_e2c = sharedVar.getArray("e" + e2c)
    var w_rc = sharedVar.getArray("r" + relc)
    var w_hprc = sharedVar.getArray("A" + relc)
    
    val X1 = get_residual(w_e1,w_e2,w_r,w_hpr)
    val X2 = get_residual(w_e1c,w_e2c,w_rc,w_hprc)
    
    var delta_1 = new Array[Double](dim.toInt)
    var delta_2 = new Array[Double](dim.toInt)
    var delta_3 = new Array[Double](dim.toInt)
    ...
    if(sum1+margin  > sum2 ){
      ...
      for(i <- 0 to dim-1){
        if(2*X1(i)>0)sign=1.0 else sign = -1.0
